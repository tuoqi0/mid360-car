...
class PathManager:
    def __init__(self):
        ...
        self.keyboard_points_added = 0  # 记录通过键盘添加的点数
        ...
        rospy.loginfo("键盘监听已启动 - 第一次标点: 三次空格键 | 第二次标点: 三次Enter键")
    ...
    def keyboard_listener(self):
        ...
        try:
            while not rospy.is_shutdown():
                ...
                # 如果已经添加了两个点，停止监听键盘输入
                if self.keyboard_points_added >= 2:
                    rospy.loginfo("已添加两个键盘点，停止键盘监听")
                    break
                ...
                if use_termios:
                    ...
                    if rlist:
                        ...
                        # 检测空格键 - 只用于第一次标点
                        if key == ' ' and self.keyboard_points_added == 0:
                            self.handle_space_key()
                        # 检测Enter键 - 只用于第二次标点
                        elif (key == '\n' or key == '\r') and self.keyboard_points_added == 1:
                    ...
                    try:
                        ...
                        if line:
                            # 检查空格键 - 只用于第一次标点
                            if ' ' in line and self.keyboard_points_added == 0:
                                self.handle_space_key()
                            # 检查Enter键 - 只用于第二次标点
                            elif (line.endswith('\n') or line.endswith('\r')) and self.keyboard_points_added == 1:
                                self.handle_enter_key()
                    except Exception as e:
    ...
    def handle_space_key(self):
        ...
        if self.space_count >= 3:
            ...
            self.keyboard_points_added = 1
            self.space_count = 0  # 重置计数器

    def handle_enter_key(self):
        """处理Enter键按下事件 - 用于第二次标点"""
        current_time = time.time()
