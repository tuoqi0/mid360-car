...
bool runtime_pos_log = false, pcd_save_en = false, time_sync_en = false, extrinsic_est_en = false, path_en = false;
/**************************/
double yaw_angle;
...
bool lidar_pushed, flg_first_scan = true, flg_exit = false, flg_EKF_inited,flg_reset;
    ...
    if (pcd_save_en)
    {
        ...
        *pcl_wait_save += *laserCloudWorld;
        ...
        if (pcl_wait_save->size() > 0 && pcd_save_interval > 0 && scan_wait_num >= pcd_save_interval)
        {
            ...
            oss << "lidar_data_" << std::put_time(&tm, "%Y%m%d_%H%M%S") << "_" << pcd_index << ".pcd"; // 带时间戳的文件名
            std::string all_points_dir = std::string(ROOT_DIR) + "PCD/" + oss.str();
...
void publish_odometry(const ros::Publisher &pubOdomAftMapped)
{
    ...
    {
        // 旋转位移
        tf2::Vector3 delta(state_point.pos(0), state_point.pos(1), state_point.pos(2));
        tf2::Quaternion q_init;
        tf2::fromMsg(initial_pose.orientation, q_init);
        tf2::Vector3 delta_rotated = tf2::quatRotate(q_init, delta);

        // 合并位置
        odomAftMapped.pose.pose.position.x = initial_pose.position.x + delta_rotated.x();
        odomAftMapped.pose.pose.position.y = initial_pose.position.y + delta_rotated.y();
        odomAftMapped.pose.pose.position.z = initial_pose.position.z + delta_rotated.z();

        // 手动设置 yaw
        tf2::Quaternion q_state;
        // 直接使用手动设定的 yaw 角度（以弧度为单位）
  
        q_state.setRPY(0, 0, yaw_rad);  // 保持 pitch 和 roll 为 0
        tf2::Quaternion q_result = q_init * q_state;
        q_result.normalize();  // 确保四元数归一化
        odomAftMapped.pose.pose.orientation = tf2::toMsg(q_result);
    }
    else
    {
    ...
    yaw_angle= atan2(2.0 * (q.getW() * q.getZ() + q.getX() * q.getY()),
                            1.0 - 2.0 * (q.getY() * q.getY() + q.getZ() * q.getZ())) * 180.0 / M_PI; // 计算当前 yaw_angle
...
bool saveMapService(fast_lio_sam::save_map::Request& req, fast_lio_sam::save_map::Response& res) {
    cout << "****************************************************" << endl;
    ...
    if(req.destination.empty()) {
        saveMapDirectory = std::string(std::getenv("HOME")) + "/Downloads/FAST_LIO_MAP";
    } else {
        // 确保路径以'/'开头时为绝对路径
        saveMapDirectory = (req.destination[0] == '/') ? 
                          req.destination : 
                          std::string(std::getenv("HOME")) + "/" + req.destination;
    }

    // 2. 创建目录（如果不存在）
    if(system(("mkdir -p " + saveMapDirectory).c_str()) != 0) {
    ...
    // 3. 保存文件（添加完整路径）
    std::string trajectory_path = saveMapDirectory + "/trajectory.pcd";
    if(pcl::io::savePCDFileBinary(trajectory_path, *cloudKeyPoses3D) != 0) {
        ROS_ERROR("Failed to save: %s", trajectory_path.c_str());
    }

    // 4. 处理点云地图（示例保存SurfMap）
    pcl::PointCloud<PointType>::Ptr globalSurfCloudDS(new pcl::PointCloud<PointType>());
    downSizeFilterSurf.setInputCloud( globalSurfCloudDS);
    downSizeFilterSurf.setLeafSize(
        (req.resolution != 0) ? req.resolution : mappingSurfLeafSize,
        (req.resolution != 0) ? req.resolution : mappingSurfLeafSize,
        (req.resolution != 0) ? req.resolution : mappingSurfLeafSize
    );
    downSizeFilterSurf.filter(*globalSurfCloudDS);

    std::string surf_map_path = saveMapDirectory + "/SurfMap.pcd";
    res.success = (pcl::io::savePCDFileBinary(surf_map_path, *globalSurfCloudDS) == 0);

    // 5. 可视化
    ros::Time timeLaserInfoStamp = ros::Time().fromSec(lidar_end_time);
    publishCloud(&pubOptimizedGlobalMap, globalSurfCloudDS, timeLaserInfoStamp, "camera_init");

    cout << "Saved to: " << saveMapDirectory << endl;
    cout << "****************************************************" << endl;
    return true;
}
...
int main(int argc, char **argv)
{
    ...
    nh.param("yaw_angle", yaw_angle, 15.0);  // 默认为 15 度



    nh.param<bool>("publish/path_en", path_en, true);
    nh.param<bool>("publish/scan_publish_en", scan_pub_en, true); //扫描数据发布(定位关，建图开)
    nh.param<bool>("publish/dense_publish_en", dense_pub_en, true);   // 密集点云发布 (定位关，建图开)
    nh.param<bool>("publish/scan_bodyframe_pub_en", scan_body_pub_en, true); //扫描坐标系发布（定位关，建图开）
    nh.param<int>("max_iteration", NUM_MAX_ITERATIONS, 4);//迭代计算
    nh.param<string>("map_file_path", map_file_path, "");
    nh.param<string>("common/lid_topic", lid_topic, "/livox/lidar");
    nh.param<string>("common/imu_topic", imu_topic, "/livox/imu");
    nh.param<bool>("common/time_sync_en", time_sync_en, false);   //时间同步
    nh.param<double>("filter_size_corner", filter_size_corner_min, 0.2);  //角点滤波尺寸
    nh.param<double>("filter_size_surf", filter_size_surf_min, 0.2);  //平面滤波
    nh.param<double>("filter_size_map", filter_size_map_min, 0.2);   // 地图滤波
    nh.param<double>("cube_side_length", cube_len, 200);       //地图体素地图分块边长
    nh.param<float>("mapping/det_range", DET_RANGE, 50.f);     //地图感知半径
    nh.param<double>("mapping/fov_degree", fov_deg, 180);     //地图建图角度
    nh.param<double>("mapping/gyr_cov", gyr_cov, 0.1);            // IMU 的陀螺仪测量噪声协方差
    nh.param<double>("mapping/acc_cov", acc_cov, 0.1);        //IMU 的加速度测量噪声协方差
    nh.param<double>("mapping/b_gyr_cov", b_gyr_cov, 0.0001);    //陀螺仪偏置的协方差（bias noise）
    nh.param<double>("mapping/b_acc_cov", b_acc_cov, 0.0001);     //加速度协方差
    nh.param<double>("preprocess/blind", p_pre->blind, 0.01);   //中心盲点
    nh.param<int>("preprocess/lidar_type", p_pre->lidar_type, AVIA);
    nh.param<int>("preprocess/scan_line", p_pre->N_SCANS, 16); //扫描线数(mid360一般为6)
    nh.param<int>("preprocess/scan_rate", p_pre->SCAN_RATE, 10);//发送速率
    nh.param<int>("point_filter_num", p_pre->point_filter_num, 2);  //点云滤波
    nh.param<double>("preprocess/z_min",p_pre->z_min,-1.2);
    nh.param<double>("preprocess/z_max",p_pre->z_max,0.5);
    nh.param<bool>("feature_extract_enable", p_pre->feature_enabled, false);  //特征提取
    nh.param<bool>("runtime_pos_log_enable", runtime_pos_log, 0);    
    nh.param<bool>("mapping/extrinsic_est_en", extrinsic_est_en, true); //外参估计（定位关，建图开）
    ...
    nh.param<double>("mappingProcessInterval", mappingProcessInterval, 0.15);// 地图更新频率（建图0.15,定位关）
    ...
    //建图从之前帧，并继续建图都开
    //使用地图定位使用第二个，第一个不开
    nh.param<bool>("visulize_IkdtreeMap", visulize_IkdtreeMap, false);//是否在 RViz 中显示地图点云（调试用）

    // reconstruct ikdtree 
    nh.param<bool>("recontructKdTree", recontructKdTree, false);    //是否强制重建地图结构（从已有关键帧恢复）
...
//建图开，不建图关
    ...
    if (pcl_wait_save->size() > 0 && pcd_save_en)
    {
        ...
        auto tm = *std::localtime(&t);
        std::ostringstream oss;
        oss << "scans_" << std::put_time(&tm, "%Y%m%d_%H%M%S") << ".pcd"; // 带时间戳的文件名
