...
#include <std_msgs/Float32.h> // 添加Float32消息支持
#include <pcl/point_types.h>  // 添加PCL点类型支持
#include <pcl/point_cloud.h> // 添加PCL点云支持
#include <cmath>             // 添加数学函数支持
#include <limits>            // 添加数值极限支持
#include <vector>            // 添加向量支持
#include <cstdint>           // 添加整数类型支持
...
enum LID_TYPE{AVIA = 8, VELO16, OUST64, RS128}; // 修改适配于mid360
...
//   EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  Preprocess();
  Preprocess(ros::NodeHandle* nh); // 添加带NodeHandle的构造函数
  ~Preprocess();
  double z_min,z_max;
